...
export const GameProvider = ({ children, gameType }: { children: React.ReactNode, gameType: 'chess' | 'checkers' }) => {
    ...
    useEffect(() => {
        ...
        const checkGameOver = (board: any, localRoomData: GameRoom) => {
            ...
            } else { // Checkers logic remains here
        ...
        const unsubscribe = onSnapshot(roomRef, (docSnap) => {
            ...
            if (roomData.status === 'completed') {
                if (!gameOverHandledRef.current) {
                    ...
                    setGameState(p => ({...p, boardState: roomData.boardState, gameOver: true, winner: roomData.draw ? 'draw' : (winnerIsMe ? 'p1' : 'p2'), gameOverReason: winnerData?.method || null, payoutAmount: myPayout }));
                }
                return;
            }
            ...
            setGameState(p => ({ ...p, playerColor: pColor, boardState: currentBoardState, moveHistory, moveCount, currentPlayer: roomData.currentPlayer, capturedByPlayer: isCreator ? roomData.capturedByP2 || [] : roomData.capturedByP1 || [], capturedByBot: isCreator ? roomData.capturedByP1 || [] : roomData.capturedByP2 || [], p1Time: isCreator ? p1ServerTime : p2ServerTime, p2Time: !isCreator ? p1ServerTime : p2ServerTime }));
            if (!isMounted) setIsMounted(true);
        });
        return () => unsubscribe();
    }, [isMultiplayer, roomId, user, isMounted, gameType, setWinner, handlePayout]);


    const loadGameState = (state: GameState) => { setGameState(state); };
    ...
    const checkGameOver = (board: any) => {}; // No longer needed here, logic moved into useEffect

    const resign = () => { if (gameState.gameOver || gameState.isEnding || !user) return; if (isMultiplayer && room) { const winnerId = room.players.find((p)=>p !== user.uid) || null; setWinner(winnerId, gameState.boardState, 'resign', user.uid, gameState.playerPieceCount); } else { setWinner('bot', gameState.boardState, 'resign', user.uid); } };
    const resetGame = () => { gameOverHandledRef.current = false; if(typeof window !== 'undefined' && !isMultiplayer) { localStorage.removeItem(storageKey); } const defaultState = getInitialState(); setGameState(defaultState); setRoom(null); };
