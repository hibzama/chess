...
export const GameProvider = ({ children, gameType }: { children: React.ReactNode, gameType: 'chess' | 'checkers' }) => {
    ...
    const handlePayout = useCallback(async (winnerDetails: { winnerId: string | 'draw' | null, method: GameOverReason, resignerDetails?: {id: string, pieceCount: number} | null }) => {
        ...
        try {
            ...
            // The payout amount is now determined server-side, we just need to calculate what our portion was.
            let myPayout = 0;
            const wager = room?.wager || 0;
            if(winnerDetails.method === 'resign' && winnerDetails.resignerDetails) {
                 myPayout = winnerDetails.resignerDetails.id === user.uid ? wager * 0.75 : wager * 1.05;
            } else if (winnerDetails.winnerId === 'draw') {
                myPayout = wager * 0.9;
            } else if (winnerDetails.winnerId === user.uid) {
                myPayout = wager * 1.8;
            } else {
                myPayout = 0; // Loss
            }
            ...
            console.error("Payout function failed:", error);
            return { myPayout: 0 };
        }
    }, [user, roomId, gameType, room?.wager]);
